...
class Folder extends Share{
    ...
    public function createNew($name='New Folder'){
        if(isset($_SESSION['username']) and strlen($name) <= 45){
            $query = "INSERT INTO `ezhil_apicourse`.`folders` (`name`, `owner`) VALUES ('$name', '$_SESSION[username]');";
    ...
    public function rename($name){
        if($this->id){
            $query = "UPDATE `ezhil_apicourse`.`folders` SET `name` = '$name' WHERE (`id` = '$this->id');";
            $result = mysqli_query($this->db, $query);
            $this->refresh();
            return $result;
        } else {
    ...
    public function delete(){
        if(isset($_SESSION['username']) and $this->getOwner() == $_SESSION['username']){
            ...
            if($this->id){
                $query = "DELETE FROM `ezhil_apicourse`.`folders` WHERE (`id` = '$this->id');";
                $result = mysqli_query($this->db, $query);
                return $result;
            } else {

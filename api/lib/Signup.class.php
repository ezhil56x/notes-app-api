...
class Signup {
    ...
    public function __construct($username, $password, $email){
        ...
        $query = "INSERT INTO `ezhil_apicourse`.`auth` (`username`, `password`, `email`, `active`, `token`) VALUES ('$username', '$password', '$email', 0, '$token');";
    ...
    public static function verifyAccount($token){
        $query = "SELECT * FROM ezhil_apicourse.auth WHERE token='$token';";
        ...
        if($result and mysqli_num_rows($result) == 1){
            ...
            mysqli_query($db, "UPDATE `ezhil_apicourse`.`auth` SET `active` = '1' WHERE (`token` = '$token');");
            return true;
        } else {
            return false;
        }
